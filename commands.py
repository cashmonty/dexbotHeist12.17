...
async def chart(ctx, token_address: str, interval: str = '1h', max_size: int = 100):
...
async def chartichi(ctx, token_address: str,  interval: str = '1h',  max_size: int = 100):
    ohlc_data = await get_ohlc_data(token_address, interval, max_size)
    if ohlc_data is not None:
...
async def chartdonchian(ctx, token_address: str,  interval: str = '1h',  max_size: int = 100):
    ohlc_data = await get_ohlc_data(token_address, interval, max_size)
    if ohlc_data is not None:
